...
/turf/unsimulated/mineral/perspective_rock
	...
	icon = 'F_40kshit/icons/turfs/walls.dmi'
...
/*	This unironically works at adding shadow borders, but fuck that shit yo.
	var/image/img = image('F_40kshit/icons/turfs/turf_outline_overlays.dmi',"main",layer = SIDE_LAYER)
	var/offset=-4
	...
	if(!istype(T1,src))
		...
		if(istype(T1T2,src))
			...
			decision1 = "bot_right"
		var/image/img1 = image('F_40kshit/icons/turfs/turf_outline_overlays.dmi',decision1,layer = SIDE_LAYER)
	...
	if(!istype(T2,src))
		...
		if(istype(T1T2,src))
			...
			decision2 = "bot_left"
		var/image/img2 = image('F_40kshit/icons/turfs/turf_outline_overlays.dmi',decision2,layer = SIDE_LAYER)
