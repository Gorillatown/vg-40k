...
/obj/item/ork/jumppack/proc/burstrush(mob/living/user)
	...
	burning = TRUE
	update_icon()
	playsound(loc, 'F_40kshit/sounds/Jump_Pack1.ogg', 75, 0)
	...
	for(var/i = 1 to wallcrashiterations) //We just loop this way.
		...
		playsound(loc, 'F_40kshit/sounds/Jump_Pack2.ogg', 75, 0)
		user.Move(get_step(usr,movementdirection), movementdirection)
		sleep(2)

	sleep(3)
	playsound(loc, 'F_40kshit/sounds/Jump_Pack3.ogg', 75, 0) //We end our shit
