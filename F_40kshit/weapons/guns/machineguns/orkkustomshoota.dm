...
/obj/item/weapon/taperoll
	...
	icon = 'F_40kshit/icons/obj/orks/kustomgun.dmi'
	icon_state = "tape"
	item_state = "tape"
...
/obj/item/weapon/gun/projectile/kustomshoota
	...
	inhand_states = list("left_hand" = 'F_40kshit/icons/inhands/LEFTIES/64x64kustom_shoota_left.dmi', "right_hand" = 'F_40kshit/icons/inhands/RIGHTIES/64x64kustom_shoota_right.dmi')
	...
	fire_sound = 'F_40kshit/sounds/Shoota1.ogg'
	recoil = 16
	load_method = 2
	force = 15
...
/obj/item/weapon/gun/projectile/kustomshoota/attackby(obj/item/I, mob/user)
	...
	if(istype(I, /obj/item/weapon/taperoll))
		...
		playsound(loc, 'F_40kshit/sounds/tape.ogg', 50, 0)
		return
			...
			if(laserbeams >= 0) //Laserbeam shit
				...
				for(var/i=1 to laserbeams)
					if(fucked_fire)
						targloc = get_step(user,turn(user.dir,rand(0,360)))
					in_chamber = new projectile_type(loc)
					fire_sound = 'F_40kshit/sounds/Lasgun0.ogg'
					Fire(targloc, user, params, struggle)
					sleep(1)
			if(shotgunpellets >= 0)
				in_chamber = null
				...
				for(var/i=1 to shotgunpellets)
					if(fucked_fire)
						targloc = get_step(user,turn(user.dir,rand(0,360)))
					in_chamber = new projectile_type(loc)
					fire_sound = 'F_40kshit/sounds/shotta.ogg'
					Fire(targloc, user, params, struggle)
					sleep(1)
		sleep(8)
	cooldown = 0
